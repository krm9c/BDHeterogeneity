...
##############################################################################################
    def Noise_optimizer(self, lr):
        ...
        cost_clamp, weight_clamp, bias_clamp = self.Layer_cost_Clamp[0]
        real_cost =  tf.nn.l2_loss(cost - cost_clamp)       
        dir_weight_update = ((tf.gradients(real_cost, weight)[0])
                            -(tf.gradients(real_cost, weight_clamp)[0]))
        dir_weight_update = tf.truediv( dir_weight_update, (tf.norm(dir_weight_update) + 0.001))
        weight_update     = -1*(tf.gradients(self.classifier["Overall cost"], weight)[0]-0*dir_weight_update)+0.0001*weight
...
##################################################################################
    def Optimizer(self, lr, dError_dy):
        ...
        for i in xrange(0, (len(self.Layer_cost)) ):
            cost, weight, bias, fan_in, batch_size, layer_in, dlayer,\
                backward = self.Layer_cost[i]
            ...
            if i < (len(self.Layer_cost) - 1):
                ...
                dir_weight_update = ((tf.gradients(self.classifier['real_cost'][i-1], weight)[0])
                                     - (tf.gradients(self.classifier['real_cost'][i-1], weight_clamp)[0]))
                self.classifier['updates'].append(
                    tf.nn.l2_loss(dir_weight_update))
                dir_weight_update = tf.truediv( dir_weight_update, (tf.norm(dir_weight_update) + 0.001))
                dir_bias_update = ((tf.gradients(self.classifier['real_cost'][i-1], bias)[0])\
                - (tf.gradients(self.classifier['real_cost'][i-1], bias_clamp)[0]))
                dir_bias_update = tf.truediv(dir_bias_update, (tf.norm(dir_bias_update) + 0.001))
                weight_update = tf.add((upd_weight),  0.001*dir_weight_update) + 0.0001*weight
                bias_update   = tf.add((upd_bias[0]), 0.001*dir_bias_update)   + 0.0001*bias
    ...
    def Custom_Optimizer(self, lr):
        train_list = []
        for i in xrange(1, len(self.Layer_cost)):
            ...
            for i in range(2, len(Layers)):
                self.Deep['FL_layer_1' + str(i)], self.Deep['dFL_layer_1' + str(i)]\
                    = self.nn_layer(self.Deep['FL_layer_1' + str(i-1)], Layers[i-1],
                                    ...
                                    trainability=False, key='FL_layer_1' + str(i))
                fan_in = Layers[i - 2]
                fan_out = Layers[i - 1]
                num_final = classes
