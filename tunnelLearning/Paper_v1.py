...
import Paper_4_class as NN_class
...
def Analyse_custom_Optimizer_GDR_old(X_train, y_train, X_test, y_test, kappa):
    ...
    X_test = sample_Z(X_test, X_test.shape[0], X_test.shape[1], kappa = 0.1)
    ...
    try:
        ...
        for i in tqdm(t):
            ...
            if i % 1 ==0:
                temp_acc = model.sess.run([model.Evaluation['accuracy'], model.classifier["Overall cost"],\
                model.classifier["Cost_Noise"]], feed_dict=return_dict(model, X_test, y_test, lr))

                train_acc = model.sess.run([model.Evaluation['accuracy']],\
                feed_dict=return_dict(model, X_train, y_train, lr) )
                
                print(i, "Train acc", train_acc, "Test Accuracy",temp_acc[0], " Overall cost", temp_acc[1], "Noise cost",  temp_acc[2])
...
print("Details", filename, "MNIST Grad -- New Framework", iterat_kappa,Train_batch_size, Train_Glob_Iterations)
Results[:, 0] = Kappa_s
