...
##############################################################################################
    def Custom_Optimizer(self, lr, dError_dy):
        ...
        for i in xrange(len(self.Layer_cost)):
            ...
            weight_update = tf.gradients(self.classifier['Overall cost'], weight)[0]
            bias_update   = tf.gradients(self.classifier['Overall cost'], bias)[0]

            # Generate the updated variables
            train_list.append((weight_update, weight))
            train_list.append((bias_update, bias))

        return tf.train.AdamOptimizer(0.001).apply_gradients(train_list)

################# Noise optimizer###############
    def Noise_optimizer(self, lr, dError_dy):
        train_list =[]
    ...
    def Optimizer(self, lr, dError_dy):
        ...
        for i in xrange(len(self.Layer_cost)):
            ...
            if i < (len(self.Layer_cost) - 1):
                ...
                #### Gradient Descent update
                dir_weight_update = ((tf.gradients(self.classifier['real_cost'][i], weight)[0])
                                     - (tf.gradients(self.classifier['real_cost'][i], weight_clamp)[0]))

                self.classifier['updates'].append(
                    tf.nn.l2_loss(dir_weight_update))

                dir_weight_update = tf.truediv( dir_weight_update, (tf.norm(dir_weight_update) + 0.001))

                dir_bias_update = ((tf.gradients(self.classifier['real_cost'][i], bias)[0])\
                - (tf.gradients(self.classifier['real_cost'][i], bias_clamp)[0]))
                dir_bias_update = tf.truediv(dir_bias_update, (tf.norm(dir_bias_update) + 0.001))

                weight_update = tf.add((upd_weight),  0.01*dir_weight_update) + 0.0001*weight
                bias_update   = tf.add((upd_bias[0]), 0.01*dir_bias_update)   + 0.0001*bias
            ...
            train_list.append(
                (tf.placeholder("float32", shape=bias_update.get_shape())))
...
#############################################################################################
#############################################################################################
    def Changing_Network(self, classes, Layers, act_function, batch_size, back_range = 1):
        i =1
        with tf.name_scope("Trainer_Network"):
            ...
            input_noise = tf.Variable(tf.random_uniform([1, Layers[0]], minval= -0.1, maxval= 0.1, dtype=tf.float32))
        ...
        with tf.name_scope("Classifier"):
            self.classifier['class_1'], self.classifier['dclass_1'] = \
...
#################### Layer wise information for generating the updates
            ...
            weight_temp = self.classifier['Weight' +
                                          'FL_layer_1' + str(len(Layers) - 1)]
            bias_temp = self.classifier['Bias' +
                                        'FL_layer_1' + str(len(Layers) - 1)]
        ...
        for i in range(1, len(Layers)):
            ...
            self.Deep['FL_layer_3' +
                      str(i)] = act_function(preactivate, name='activation_3' + key)
        self.classifier['class_3'] = tf.identity(tf.matmul(self.Deep['FL_layer_3' + str(
    ...
    def Clamped_Network(self, classes, Layers, act_function, batch_size):
    # The network where everything is assigned and we do the calculations and predictions here.
    #####################################################################################################
    ##########################################Clamped Network############################################
        with tf.name_scope("clamped"):
            i = 1
            self.Deep['FL_layer_20'] = self.Deep['FL_layer_10'] 
            self.Deep['FL_layer_21'], self.Deep['dFL_layer_21'] \
                = self.nn_layer(self.Deep['FL_layer_20'], Layers[i - 1], Layers[i], act=act_function,
                                trainability=False, key='FL_layer_2' + str(i))
            # Neural network for the rest of the layers.
            for i in range(2, len(Layers)):
                self.Deep['FL_layer_2' + str(i)], self.Deep['dFL_layer_2' + str(i)]\
                    = self.nn_layer(self.Deep['FL_layer_2' + str(i - 1)], Layers[i - 1],
                                    Layers[i], act=act_function, trainability=False, key='FL_layer_2' + str(i))
    ##################################################### Layer 1-d-1 #######################################################################
    ################################# Layer wise information for generating the updates   
                cost_layer = self.Deep['FL_layer_2' + str(i)]
                weight_temp = self.classifier['Weight' +
                                              'FL_layer_2' + str(i - 1)]
                bias_temp = self.classifier['Bias' + 'FL_layer_2' + str(i - 1)]
                self.Layer_cost_Clamp.append(
                    (cost_layer, weight_temp, bias_temp))
        with tf.name_scope("Classifier"):
            self.classifier['class_2'], self.classifier['dclass_2'] =\
            self.nn_layer(self.Deep['FL_layer_2' + str(len(Layers) - 1)],\
            Layers[len(Layers) - 1], classes, act=tf.identity, trainability=False, key='class_2')

    ##################################################### Layer d-1 #######################################################################
    #################### Layer wise information for generating the updates 
            cost_layer = self.classifier['class_2']
            weight_temp = self.classifier['Weight' +\
            'FL_layer_2' + str(len(Layers) - 1)]
            bias_temp = self.classifier['Bias' +\
            'FL_layer_2' + str(len(Layers) - 1)]
            self.Layer_cost_Clamp.append( (cost_layer, weight_temp, bias_temp) )
...
########################################################## The network that changes $$ Class 1
            self.Changing_Network(classes, Layers, act_function, batch_size)
##############################################################################################
################################################# Design the trainer for the unclamped network
        with tf.name_scope("Trainer"):
            ######################## Two types of costs for learning
            # L2 Loss
            # self.classifier["cost_NN"] = tf.nn.l2_loss(
            ...
            self.classifier["cost_NN_Without_Noise"] = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
                logits=self.classifier['class_3'], labels=self.classifier['Target'], name='Error_Cost'))
            # Noise loss 
            # L2 Loss
        ...
        #################### Layer wise information for generating the updates
            ...
            # The network that is held constant $$ Class 2
            self.Clamped_Network(classes, Layers, act_function, batch_size)
    
    ##############################################################################################
    ######################## Two types of costs for learning
            # L2 Loss
            # self.classifier["cost_NN_clamp"] = tf.nn.l2_loss( self.classifier['class_2'] - self.classifier['Target'] )  
            # Cross Entropy Loss
            self.classifier["cost_NN_clamp"] = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=self.classifier['class_2'], labels=self.classifier['Target'], name='Error_Cost_clamped'))
    
    ##################################################### The cost for the unclamped network ##############################################      
            cost_layer = self.classifier["cost_NN_clamp"]
            weight_temp = self.classifier['Weightclass_2']
            bias_temp = self.classifier['Biasclass_2']
            self.Layer_cost_Clamp.append((cost_layer, weight_temp, bias_temp))

            # The final cost           
            self.classifier["Overall cost"] =\
            0.1*self.classifier["cost_NN"] + 0.9*self.classifier["cost_NN_Without_Noise"]\
            -[0.001*self.classifier["Cost_Noise"]]
##########################################################################################################
# The final optimization
        with tf.name_scope("Trainers"): 
            # The general optimizer
            dError_dy = tf.reshape(tf.gradients(
            self.classifier["Overall cost"], self.classifier['class_1'])[0], [batch_size, 1, num_final])

            # the optimizers
            self.Trainer["grads"], self.Trainer["grad_placeholder"], self.Trainer['optimization_op'] = self.Optimizer(
            self.classifier["learning_rate"],  dError_dy)
            
            # Call the other optimizer
            self.Trainer["apply_placeholder_op"] =\
            self.Custom_Optimizer(
            self.classifier["learning_rate"],  dError_dy)

            # Call the Noise optimizer
            self.Trainer["Noise_op"] = self.Noise_optimizer(
            self.classifier["learning_rate"],  dError_dy)
            
            # Assign reassign all the weights
            self.Trainer['assign_op'] = self.assign()
