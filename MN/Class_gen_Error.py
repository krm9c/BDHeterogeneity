...
#############################################################################################
    def gen_feedback(self, layer_grad,  layer_num_nodes, out_num_nodes):
        with tf.variable_scope("radom_matrix_1"):
            ...
            Bhat = tf.Variable(tf.random_uniform([layer_num_nodes, out_num_nodes], -1, 1)) + layer_grad
            return Bhat
# #############################################################################################
        ...
        with tf.variable_scope("direct_feedback_alignment"):
            ...
            for i, (layer_out, layer_weights) in enumerate(activation_param_pairs):
                with tf.variable_scope("virtual_feedback_{}".format(i)):
                    ...
                    print("The shape of the layer is")
                    ...
                    Feedback = tf.matmul(loss_grad_shape, tf.transpose(B)) 
                    su = 0

                    for weight in layer_weights:
                        if su>0:
                            Delta_w_temp = tf.reduce_mean(tf.matmul(tf.transpose(Feedback), layer_out),1)    
                        else:
                            Delta_w_temp = tf.matmul(tf.transpose(Feedback), layer_out)
                        
                        Delta_w = tf.reshape(Delta_w_temp, tf.shape(weight))
                        virtual_gradient_param_pairs +=[(Delta_w , weight)]
                        su = su+1
