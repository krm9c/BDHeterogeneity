...
def Analyse_GDR_with_Hetero(X_train, y_train, X_test, y_test, kappa):
    ...
    tf.reset_default_graph()
    gc.collect()
    print("iteration Accuracy -- Test",
          acc_array[i],  "Train", acc_array_train[i], "gen_error", (acc_array[i] - acc_array_train[i]))
...
def Analyse_EDL(X_train, y_train, X_test, y_test, kappa):
    ...
    try:
        ...
        for i in tqdm(t):
            for batch in iterate_minibatches(X_train, y_train, Train_batch_size, shuffle=True):
                batch_xs, batch_ys = batch
                batch_xs_noise = sample_Z(
                    batch_xs, batch_xs.shape[0], batch_xs.shape[1], 1)
                model.sess.run([model.Trainer["EDL"]], feed_dict=return_dict_EDL(
                    model, batch_xs, batch_ys))
                model.sess.run([model.Trainer["EDL"]], feed_dict=return_dict_EDL(
                    model, batch_xs_noise, batch_ys))
...
#####################################################################################
### Final Loop
print("Four Layers", "Details", filename, dataset,
      iterat_kappa, Train_batch_size, Train_Glob_Iterations)
